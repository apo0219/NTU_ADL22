...
def main(args):
    ...
    model = Slot_tag(args.input_size, args.hidden_size, args.dropout, 9, 1, args.num_layers, args.model_type)
    model.eval()
    ...
    for batch in dataloader:
        id = batch["id"][0]
        token_lst = batch['tokens'][0].split(' ')
        data = []
        for j in token_lst:
            if (j not in vocab.token2idx):
                data.append(torch.zeros([300], dtype = torch.float64))
                continue
        ...
        y = model(pack_padded_x.float())
        y = y[0].argmax(1).tolist()
        out_tags = []
        for i in y:
            out_tags.append(idx2tag[i])
        out_tags = " ".join(out_tags)
        outdata.append([id, out_tags])
...
def parse_args() -> Namespace:
    ...
    parser.add_argument(
        ...
        default="./ckpt/slot/super_big_slot.pt",
    )
