...
class DataTrainingArguments:
    ...
    output_file: Optional[str] = field(
        default=False, metadata={"help": "dump output into output_file"}
    )
...
def main():
    ...
    def preprocess_function(examples):
        ...
        second_sentences = [
            [examples[end][i] for end in ending_names] for i in range( len( examples[context_name] ) )
        ]
    ...
    if training_args.do_predict:
        ...
        pb = results.predictions
        ans_idx = np.argmax(pb,axis=1)
        # print(pb)
        print(f"------------ open {data_args.test_file} -------------")
        with open( data_args.output_file, 'w' ) as f:
            # print("suc")
            for ans in ans_idx:
                # print(ans, ',')
                f.write(str(ans) + ',')
        # print(format(np.argmax(pb,axis=1)))
